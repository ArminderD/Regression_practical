...
Descriptive statistics will be presented as mean and standard deviations for continuous Exploratory data analysis will involve obtaining correlation coefficients for continuous explanatory variables (Pearson correlation if variables are approximately normally distributed or the non-parametric Spearman correlation coefficient if either of the variables are not normally distributed), ttests for binary explanatory variables (or a non-parametric equivalent such as the Mann Whitney test if the outcome is not normally distributed) and ANOVA for categorical variables or the non-parametric Kruskal-Wallis test). 

To adjust for potential confounders, in addition to blood lead level ion 1972, age, sex, area of residence and whether or not the child lived within 1 mile of the smelter for the first two years of their life will then be included in a multiple linear regression model. Because blood lead level in 1973 was obtained at a later time to the other variables, this measure was not considered appropriate for inclusion in the model. It is also likely to be highly correlated with blood lead level in 1972. Finger wrist tapping score was also not considered as an explanatory variable as this is a neurological measure, and clinically / biologically not meaningful to include in this model. 
...
# Add a Normal Curve 
x <- age
h<-hist(x, xlab="Age in Years", breaks = 10, main="Histogram with Normal Curve") # prob = T for density
...
The results of the t tests do not indicate a relationship between IQ and sex, area of residence and whether the child lived within 1 mile of the smelter for the first two years.
...
To plot density curve for residuals,install the `MASS` package.
...
IQ does not seem to be associated with blood lead level. While IQ does decrease slightly with increasing blood lead level, this is not statistically significant. The 95% CI for the coefficient estimate includes 0 and the P value for the test that the coefficient is 0 is < 0.05 (t = -1.35, p = 0.18).
